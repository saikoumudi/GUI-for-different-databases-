		...
		if(session == null) {
			...
			if(error_occured)	writer.println("<H3>" + error_message + "</H3></BODY></HTML>");
				...
				if(largeColumnCount == 0) {
					writer.println("<FORM NAME=edit_form METHOD=post ACTION='/httpadmin/EditNorm'>");
					...
					writer.println("<TR>");
					writer.println(	"<TD WIDTH=20% ALIGN=left><IMG SRC='/httpadmin/pics/go1.jpg' " +
					...
					writer.println(	"<TD WIDTH=20% ALIGN=right><IMG SRC='/httpadmin/pics/go1.jpg' " +
					...
					for(int i=0;i<rowIds.length;i++) {
						...
						writer.println(	"<FORM METHOD=post ENCTYPE='multipart/form-data' " +
										" NAME='edit_form" + i + "' ACTION='/httpadmin/EditLobs'>");
						writer.println("<INPUT TYPE=hidden NAME=table_name VALUE=" + tableName + ">");
						...
						for(int j=0;j<columnCount;j++)	{
							...
							if(columnCount == j+1) {
								...
								writer.println("<TD WIDTH=5%><IMG SRC='/httpadmin/pics/go1.jpg' " +
										"NAME=go1 WIDTH=30 HEIGHT=30 BORDER=0 STYLE='cursor:hand' " +
										"onMouseOut='putOff(this,13)' onMouseUp='putOff(this,13)' " +
