		...
		if(session == null) {
			...
			if(error_occured) {
				...
				writer.println(	"<TR><TD>&nbsp&nbsp&nbsp&nbsp" +
								...
								"<img name=pic3 src='/httpadmin/pics/browse2.jpg' border=0 " +
								...
								"width=80 height=26 align=absbottom></A>" +
								"<A HREF='/httpadmin/TabOperations?table_name=" + tableName + "'>" +
				...
				try {
					...
					writer.println(	"<TH id=browse_hed WIDTH=4%>" +
									"<IMG SRC='/httpadmin/pics/button_edit.png' WIDTH=12 HEIGHT=13 " +
									" BORDER=0 ALT='Edit' STYLE='cursor:hand' " +
									" onClick='submitEditForm(action_form,action_form.row_ids)'></TH>");
					writer.println(	"<TH id=browse_hed WIDTH=4%>" +
									"<IMG SRC='/httpadmin/pics/button_drop.png' WIDTH=12 HEIGHT=13 " +
					...
					if(startIndex > 1) {
						writer.println("<TH id=browse_hed WIDTH=5%><IMG BORDER=0 WIDTH=12 HEIGHT=12 " +
									   "SRC='/httpadmin/pics/prev.gif' onClick='prev_form.submit()' " +
									   " style='cursor:hand' ALT='Previous " + recordsPerPage +
									   " record(s)'></TH>");
						
						writer.println("<FORM NAME=prev_form METHOD=post ACTION='/httpadmin/BrowseTab'>");
					...
					if((startIndex + recordsPerPage - 1) < rowCount) {
						writer.println(	"<TH id=browse_hed WIDTH=5%><IMG BORDER=0 WIDTH=12 HEIGHT=12 " +
										" SRC='/httpadmin/pics/next.gif' style='cursor:hand' " +
										" onClick='next_form.submit()' ALT='Next " + recordsPerPage +
										" record(s)'></TH>");
						writer.println("<FORM NAME=next_form METHOD=post ACTION='/httpadmin/BrowseTab'>");
						writer.println("<INPUT TYPE=hidden NAME=row_count VALUE='" + rowCount + "'>");
						writer.println(	"<INPUT TYPE=hidden NAME=records_per_page VALUE='" + 
										recordsPerPage + "'>");
						writer.println(	"<INPUT TYPE=hidden NAME=start_index VALUE='" + 
										...
										if(index != -1) {
											...
											if(type.equalsIgnoreCase("NONE")) {
												writer.println(	"<TD id=browse_td>UNKNOWN DATA FORMAT " +
																"<A HREF='/httpadmin/LoadLob?tname=" +
																tableName + "&cname=" + 
																selectedColumns[i] +
																...
																sortOrder + "'> " +
																"<IMG SRC='/httpadmin/pics/load.gif' " +
